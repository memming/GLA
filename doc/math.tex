...
\begin{align}
    &p(y|x,\hyp) \nonumber\\
    &\quad=
    \frac{|2\pi\Sigma|^{\frac{1}{2}}}
    {|2\pi C|^{\frac{1}{2}} |2\pi \Phi|^{\frac{1}{2}}}
    \exp\left(
	-\frac{1}{2}
	\left[
	    b\trp C\inv b + y\trp\Phi\inv y
	    - (C\inv b + x\Phi\inv y)\trp 
		\Sigma
		(C\inv b + x\Phi\inv y)
	\right]
    \right)
\end{align}
where $\Sigma = \left( C\inv + x\Phi\inv x\trp \right)\inv$.

The usual special case is when $b = 0$ and $\Phi = \sigma^2 I$ which gives,
\begin{align}
    p(y|x,\hyp) =
    \frac{|2\pi\Sigma|^{\frac{1}{2}}}
    ...
    \right)
\end{align}
where $\Sigma = \left( C\inv + \frac{1}{\sigma^2}x x\trp \right)\inv$ which agrees with~\cite{Sahani2002}.
